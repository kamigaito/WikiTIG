...
class Dataset(data.Dataset):
    ...
    def get_filenames(self, data_path):
        ...
        for path, subdirs, files in os.walk(data_path):
            for name in files:
                if name.lower().rfind('jpg') != -1 or name.lower().rfind('jpeg') != -1 or name.lower().rfind('png') != -1 or name.lower().rfind('gif') != -1:
...
def calculate_fid_given_paths(path, batch_size, cuda, dims):
    ...
    np.savez_compressed(path + ".npz", mu=mu, sigma=sigma)

if __name__ == '__main__':
    args = parser.parse_args()
    os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu
    print(args.path)
    calculate_fid_given_paths(args.path, args.batch_size, args.gpu, args.dims)
