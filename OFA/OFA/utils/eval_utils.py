...
def eval_image_gen(task, generator, models, sample, **kwargs):
    ...
    uniq_id = ".".join(sample['id'][0].split(".")[:-1])
    ...
    if task.cfg.gen_images_path:
        ...
        caption = task.bpe.decode(task.tgt_dict.string([token for token in caption_tokens if token >= 4]))[
                  38:].replace('/', '')
        """
        task.dump_images(sorted_hyps, text=uniq_id, path=os.path.join(task.cfg.gen_images_path, 'all_results'))
        task.dump_images(sorted_hyps, text=uniq_id, path=os.path.join(task.cfg.gen_images_path, 'top1'), topk=1)
...
def eval_gigaword(task, generator, models, sample, **kwargs):
    ...
    results = []
    for i in range(len(gen_out)):
        hyp = decode_fn(gen_out[i][0]["tokens"], task.tgt_dict, task.bpe, generator).strip()
