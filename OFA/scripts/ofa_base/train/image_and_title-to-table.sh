...
log_dir=${ROOT}/logs/ofa_base/image_and_title-to-table
save_dir=${ROOT}/models/ofa_base/image_and_title-to-table
...
data_dir=${ROOT}/../datasets/image-to-text/480/ease/image-to-table
  ...
  for warmup_ratio in {0.06,}; do
    ...
    for drop_worst_after in {6000,}; do
      ...
      for seed in {0,1,2,}; do
        ...
        log_file=${log_dir}/${max_epoch}"_"${warmup_ratio}"_"${drop_worst_after}"_"${seed}".log"
        save_path=${save_dir}/${max_epoch}"_"${warmup_ratio}"_"${drop_worst_after}"_"${seed}
