...
with open(BASE64_FILE) as f_in, open(OUT_DIR_1 + "/all.tsv", "w") as f_out_exact, open(OUT_DIR_2 + "/all.tsv", "w") as f_out_ease:
    for line in f_in:
        img, b64 = line.strip().split("\t")
        ...
        # title caption table img base64
        columns = [title,decite(caption),table,img,b64]
