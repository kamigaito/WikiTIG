...
def line_format(f_out, title, table):
    f_out.write("\t".join([title, table]) + "\n")
...
with open(FILE_IN) as f_in, open(test_file, "w") as f_test, open(valid_file, "w") as f_valid, open(train_file, "w") as f_train:
    for line in f_in:
        ...
        if title_hash % SPLIT_SIZE == 0:
            line_format(f_test, title, table)
        elif title_hash % SPLIT_SIZE == 1:
            line_format(f_valid, title, table)
        else:
            line_format(f_train, title, table)
